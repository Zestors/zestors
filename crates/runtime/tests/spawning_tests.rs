...
//! (src/spawn.rs): pid handling, the four entry points, respawning on a
...
#[tokio::test]
async fn spawn_uses_the_given_pid() {
    let pid = common::test_pid("spawn_specific_pid");
...
#[tokio::test]
async fn spawn_rand_uses_a_fresh_random_pid() {
...
#[tokio::test]
async fn spawn_and_spawn_task_reject_a_duplicate_pid() {
    let pid = common::test_pid("spawn_dup");
    let child = spawn(pid.clone(), common::simplest_handler).unwrap();
...
#[tokio::test]
async fn spawn_on_a_strong_address_registers_the_pid_it_was_created_with() {
    let pid = common::test_pid("strong_spawn_registers");
    let strong = StrongAddress::create(pid.clone()).unwrap();
...
#[tokio::test]
async fn respawning_reuses_the_same_channel_and_pid() {
    let strong = StrongAddress::create(common::test_pid("respawn_same_channel")).unwrap();
...
async fn cannot_spawn_again_while_a_previous_run_is_still_active() {
    let strong = StrongAddress::create(common::test_pid("respawn_while_active")).unwrap();
...
async fn respawning_with_a_different_handler_is_allowed() {
    let strong = StrongAddress::create(common::test_pid("respawn_different_handler")).unwrap();
...
async fn a_panic_before_the_first_recv_still_registers_spawned_and_exited() {
    let strong = StrongAddress::create(common::test_pid("panic_before_recv")).unwrap();
    ...
    let result = tokio::time::timeout(std::time::Duration::from_secs(2), child).await;
    assert!(result.is_ok());
    assert_eq!(strong.status(), ActorStatus::Exited(zestors_runtime::ExitStatus::Panicked));
...
async fn spawn_error_result_maps_to_unhandled_error_exit_status() {
    ...
    assert_eq!(outcome, Err(zestors_runtime::errors::ExitError::UnhandledError));
}
...
async fn created_at_is_stable_across_respawns_while_last_spawned_at_updates() {
    let strong = StrongAddress::create(common::test_pid("created_vs_spawned")).unwrap();
    ...
    assert_eq!(child2.created_at(), created_at, "created_at is fixed for the channel's lifetime");
