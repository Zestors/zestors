...
//! | [`distr`] | Distributed messaging: nodes form a cluster, and `ClusterAddress` sends messages to actors on this node or another. `StableId` gives each message type a stable, globally unique `Id`. |
