...
use zestors_core::{
    ...
    inbox::InboxKind,
    messaging::{
        AnyMessage, Message, Protocol, ProtocolFrom, SendError, SendUncheckedError, TrySendError,
        TrySendUncheckedError,
    ...
    fn try_send_boxed(&self, boxed: AnyMessage) -> Result<(), TrySendUncheckedError<AnyMessage>> {
        match P::try_from_msg(boxed) {
            Ok(prot) => self.try_send_raw(prot).map_err(|e| match e {
                TrySendError::Full(prot) => TrySendUncheckedError::Full(prot.into_msg()),
                TrySendError::Closed(prot) => TrySendUncheckedError::Closed(prot.into_msg()),
            }),
    ...
    fn send_blocking_boxed(&self, boxed: AnyMessage) -> Result<(), SendUncheckedError<AnyMessage>> {
        match P::try_from_msg(boxed) {
            Ok(prot) => self
                ...
                .map_err(|SendError(prot)| SendUncheckedError::Closed(prot.into_msg())),
...
mod test {
    ...
    use zestors_core::config::BackPressure;
...
impl<P: Protocol> ActorKind for Inbox<P> {
