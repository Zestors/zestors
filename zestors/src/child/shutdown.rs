...
pub struct ShutdownFut<'a, E: Send + 'static, T: ActorKind> {
    child: &'a mut Child<E, T>,
    sleep: Option<Pin<Box<Sleep>>>,
}

impl<'a, E: Send + 'static, T: ActorKind> ShutdownFut<'a, E, T> {
...
impl<'a, E: Send + 'static, T: ActorKind> Unpin for ShutdownFut<'a, E, T> {}

impl<'a, E: Send + 'static, T: ActorKind> Future for ShutdownFut<'a, E, T> {
...
pub struct ShutdownStream<'a, E: Send + 'static, T: ActorKind> {
...
impl<'a, E: Send + 'static, T: ActorKind> Stream for ShutdownStream<'a, E, T> {
...
mod test {
    ...
    async fn shutdown_group_success() {
        let (mut child, _addr) = spawn_many(0..3, Config::default(), grouped_basic_actor!());
